...
@testset "Smearing for insulators" begin
    ...
    occupation0, εF0 = DFTK.compute_occupation(basis, eigenvalues)
    ...
    for temperature in Ts, smearing in smearing_methods
        ...
        occs, _ = with_logger(NullLogger()) do
            DFTK.compute_occupation(basis, eigenvalues)
        end
...
@testset "Smearing for a simple metal" begin
    ...
    for (smearing, temperature, εF_ref) in parameters
        occupation, εF = with_logger(NullLogger()) do
            DFTK.compute_occupation(basis, eigenvalues; smearing, temperature)
        end
...
if mpi_nprocs() == 1
@testset "Smearing with multiple possible Fermi levels" begin
    ...
    parameters = (
        ...
        (DFTK.Smearing.MethfesselPaxton(2), 1e-2, 0.2713096607939751),
        (DFTK.Smearing.MethfesselPaxton(5), 1e-2, 0.2724701412295064),
...
@testset "Smearing with multiple possible Fermi levels" begin
    ...
    for temperature in (1e-4, 1e-3, 1e-2), smearing in smearing_methods
        smearing isa Smearing.None && continue
        occupation, εF = DFTK.compute_occupation(scfres.basis, scfres.eigenvalues;
                                                 smearing, temperature)
        ρ = DFTK.compute_density(scfres.basis, scfres.ψ, scfres.occupation;
                                 scfres.occupation_threshold)
        @test DFTK.weighted_ksum(basis, sum.(occupation)) ≈ model.n_electrons
        @test sum(ρ) * scfres.basis.dvol ≈ model.n_electrons
    end
end
