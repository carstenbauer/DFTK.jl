...
@testset "ForwardDiff stresses on silicon" begin
    ...
    ref_scfres = FiniteDiff.finite_difference_derivative(0.0) do ε
        basis = make_basis(lattice + ε*dir*lattice, false)
        scfres = self_consistent_field(basis; is_converged=DFTK.ScfConvergenceDensity(1e-13))
...
@testset "ForwardDiff stresses on silicon with NLCC" begin
    function make_basis(lattice, symmetries)
        Si = ElementPsp(silicon.atnum, :Si, load_psp(silicon.psp_upf))
        model = model_PBE(lattice, [Si, Si], silicon.positions; symmetries)
        PlaneWaveBasis(model; Ecut=7, kgrid=(3, 3, 3))
    end
